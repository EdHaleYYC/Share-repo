...
class Agent():
        def __init__(self, alpha, beta, input_dims, tau, env, gamma=0.99,
                    updated_actor_interval=2, warmup=1000, n_actions=2, max_size=1000000, layer1_size=400,
            ...
            self.update_actor_iter = updated_actor_interval 
        ...
        def choose_action(self, observation):
            if self.time_step < self.warmup:
                mu = T.tensor(np.random.normal(scale=self.noise, size=(self.n_actions))) #adjust scale per environment
            else:
