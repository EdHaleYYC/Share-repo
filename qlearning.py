...
class StateDigitizer():
    def __init__(self, bounds=(2.4, 4, 0.209, 4), n_bins=10):
        self.position_space = np.linspace(-1*bounds[0], bounds[0], n_bins) #convert discrete to continous spaces
        self.velocity_space = np.linspace(-1*bounds[1], bounds[1], n_bins)
        self.pole_angel_space = np.linspace(-1*bounds[2], bounds[2], n_bins)
    ...
    def get_state_space(self):
        ...
        for i in range(len(self.position_space)+1):
            for j in range(len(self.velocity_space)+1):
                for k in range(len(self.pole_angel_space)+1):
    ...
    def digitize(self, observation):
        ...
        pole_theta = int(np.digitize(theta, self.pole_angel_space))
